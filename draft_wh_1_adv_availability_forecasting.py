...
class WaterHeater():
    ...
    def execute(self,Ttank, Tset, Tamb, RHamb, Tmains, hot_draw, control_signal, service_calls_accepted, Element_on, timestep, forecast_draw):
        (Ttank, Tset, Eused, Eloss, ElementOn, Eservice, SoC, AvailableCapacityAdd, AvailableCapacityShed, service_calls_accepted, is_available_add, is_available_shed) = self.WH(Ttank, Tset,Tamb,Tmains,hot_draw, control_signal, Element_on, service_calls_accepted, self.max_service_calls, timestep, forecast_draw)
        
        return Ttank, Tset, SoC, AvailableCapacityAdd, AvailableCapacityShed, service_calls_accepted, Eservice, is_available_add, is_available_shed, ElementOn
        
    def WH(self,Tlast, Tset, Tamb_ts,Tmains_ts,hot_draw_ts, control_signal_ts, Element_on_ts, service_calls_accepted_ts, max_service_calls, timestep, forecast_draw):
#############################################################################
        #        Baseline operation
...
#        isAvailable_ts = 1 if max_service_calls > service_calls_accepted_ts  else 0
 ...
 ######################################################################################      
        ...
        return Ttank_ts, Tset_ts, Eused_ts, Eloss_ts, Element_on_ts, Eservice_ts, SOC, Available_Capacity_Add, Available_Capacity_Shed, service_calls_accepted_ts, isAvailable_add_ts, isAvailable_shed_ts
